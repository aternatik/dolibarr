...
/**
 * Class for Dolibarr user webservices
 *
 * @author jfefe
 */
class wsUser extends DolWS
{
    ...
    function getUser($authentication, $id = '', $ref = '', $ref_ext = '')
    {
        ...
        if (!$error) {
            ...
            if ($fuser->rights->user->user->lire || ($fuser->rights->user->self->creer && $id && $id == $fuser->id) || ($fuser->rights->user->self->creer && $ref && $ref == $fuser->login) || ($fuser->rights->user->self->creer && $ref_ext && $ref_ext == $fuser->ref_ext)) {
                $user = new User($db);
                ...
                if ($result > 0) {
                    ...
                    $objectresp = array(
                        ...
                        'user' => parent::array_to_object(array(
                            'id' => $user->id,
